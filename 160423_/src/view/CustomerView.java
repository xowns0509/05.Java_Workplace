...
public class CustomerView extends JPanel
{
	...
	selectorTable XYZ = null;

	// 데이터베이스 연결객체 변수
	CustomerModel db;
	ArrayList<Customer> ctmrlist = new ArrayList<Customer>();
	...
	public void searchByName(){
		...
		try{
			ctmrlist = db.selectByName(tfCustNameSearch.getText());
			System.out.println("ctmrlist크기출력: "+ctmrlist.size());
			System.out.println("메인, selectByName메소드 진입. 정상적으로 db로부터 값받아옴.");

			XYZ = new selectorTable(ctmrlist);			//팝업리스트 실행
...
//		returnedName = XYZ.setData();
		
//		for(int i=0; i < ctmrlist.size(); i++){
//		
//		if(returnedName.equals(ctmrlist.get(i).getCustName())){
//
//			tfCustTel.setText(ctmrlist.get(i).getCustTel1());//tfCustTel
//			tfCustName.setText(ctmrlist.get(i).getCustName());//tfCustName
//			tfCustTelAid.setText(ctmrlist.get(i).getCustTel2());//tfCustTelAid
//			tfCustAddr.setText(ctmrlist.get(i).getCustAddr());//tfCustAddr
//			tfCustEmail.setText(ctmrlist.get(i).getCustEmail());//tfCustEmail
//		}
	}
...
//				try{
//					while(XYZ.threadSW){
//						System.out.println("selectByName 쓰레드 진입");
//
//						wait(0);
//					}
//
