...
//		catListARLT.add(catListARL);
	...
	class ButtonEventHandler implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			...
			if(evt == bTreeRefresh){//트리 갱신
				...
				}else if(   catListARLT.get(0).get(1).equals("") == true   ){
					// 0번째 칸 할당내용의 공백 확인
	...
	void addLayout(){
		...
		BevelBorder ty = new BevelBorder(BevelBorder.LOWERED);
		p_Right_Up_North.setBorder(ty);
		...
		JScrollPane p_Right_Up_Center_scroll = new JScrollPane(p_Right_Up_Center);
		...
		BevelBorder tdy = new BevelBorder(BevelBorder.LOWERED);
		p_Right_Down_North.setBorder(tdy);
		...
		TitledBorder x = new TitledBorder("현재 참여 중인 개발자");
		...
		BevelBorder z = new BevelBorder(BevelBorder.LOWERED);
		p_Right_Down_South.setBorder(z);
		...
		p_Right_Down_South.add(bSend = new JButton("전달"));
