...
public class NewAccount extends JFrame 
{
	...
	public void getDefaultInfo() throws Exception{
		...
		String sqlId = "";
		if(job.equals("개발자")){
			sqlId = "SELECT emp_id FROM emp WHERE job = 'PL'";
		}else if(job.equals("PL")){
			sqlId = "SELECT emp_id FROM emp WHERE job = 'PM'";
		}else if(job.equals("PM")){
			sqlId = "SELECT emp_id FROM emp WHERE job = '!'";//더미 
		}
		//아이디 가져오기
	...
	void signUp() throws Exception{
		...
		//프로젝트 번호 가져오기 
		String tmpSql = "SELECT p_num FROM project WHERE p_name = ?";
