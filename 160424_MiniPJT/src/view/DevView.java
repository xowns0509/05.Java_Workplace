...
import module.MakeCheckList.TreeEventHandler;
...
public class DevView extends JFrame
{
	...
	DefaultMutableTreeNode treeCompeleteRoot; //개발자 개인 체크리스트 트리 받을 놈
	DefaultMutableTreeNode treeCompeleteTop; //개발자 개인 체크리스트 트리 받을 놈
	DefaultMutableTreeNode treeCompeleteLeaf; //개발자 개인 체크리스트 트리 받을 놈
	
	DefaultMutableTreeNode treeWaitingRoot; //개발자 개인 체크리스트 트리 받을 놈
	DefaultMutableTreeNode treeWaitingTop;
	DefaultMutableTreeNode treeWaitingLeaf; //개발자 개인 체크리스트 트리 받을 놈
	
	DefaultMutableTreeNode treeDeniedRoot; //개발자 개인 체크리스트 트리 받을 놈
	DefaultMutableTreeNode treeDeniedTop;
	DefaultMutableTreeNode treeDeniedLeaf; //개발자 개인 체크리스트 트리 받을 놈
	...
	public void addLayout(){
		...
		// 체크리스트 탭 그리드
		JPanel p_Chk = new JPanel(new BorderLayout());
		...
		treeWaiting = new JTree();
		JScrollPane scroll_treeWaiting = new JScrollPane(tree = new JTree(devRoot));
		...
		treeDenied = new JTree();
		JScrollPane scroll_treeDenied = new JScrollPane(tree = new JTree(devRoot));
		...
		// 중앙상중하단 붙이기
		p_Chk_Center_Middle.add(scroll_treeWaiting);
		p_Chk_Center_Middle.add(p_Chk_Center_Middle_Row2);
		p_Chk_Center_Middle.add(scroll_treeDenied);
		
		// 오른 - 개인 체크리스트 완료된거
		JPanel p_Chk_Center_Right = new JPanel();
		treeCompelete = new JTree();
		JScrollPane scroll_treeCompelete = new JScrollPane(tree = new JTree(devRoot));
		
		// 중앙 - 그리드 붙이기
		p_Chk_Center.add(devTree);
		p_Chk_Center.add(p_Chk_Center_Middle);
		p_Chk_Center.add(treeCompelete);
		
		// 체크리스트 탭 붙이기
		p_Chk.add(p_Chk_Center, BorderLayout.CENTER);
		p_Chk.add(p_Chk_South, BorderLayout.SOUTH);
	...
	void refreshTrees(){
		...
		//승인
		treeCompeleteTop = new ArrayList<DefaultMutableTreeNode>();

		for(int i = 0 ; i < chkLARLT.size() ; i++){	//갖고온 카테고리의 갯수만큼만 반복문 돌려.

			treeCompeleteTop.add(new DefaultMutableTreeNode(String.valueOf(i)+". "+chkLARLT.get(i)));
		


			if(   (chkLARLT.get(i).get(4)).equals("승인") != false   ){
				// chkLARLT.get(i)는 i번째 레코드(행)
				// chkLARLT.get(i).get(1)는 해당 레코드(행)의 카테고리이름
				// chkLARLT.get(i).get(2)는 해당 레코드(행)의 항목내용
				// chkLARLT.get(i).get(4)는 해당 레코드(행)의 항목상태
				...
				//항목내용이 카테고리에 삽입
				treeCompeleteTop.get(i).add(new DefaultMutableTreeNode(chkLARLT.get(j).get(3)));

			}else if(   (chkLARLT.get(i).get(4)).equals("보류") != false   ){
				treeCompeleteTop.get(i).add(new DefaultMutableTreeNode(chkLARLT.get(j).get(3)));
				
			}else if(   (chkLARLT.get(i).get(4)).equals("승인대기") != false   ){
				treeCompeleteTop.get(i).add(new DefaultMutableTreeNode(chkLARLT.get(j).get(3)));
				
			}else if(   (chkLARLT.get(i).get(4)).equals("작업중") != false   ){
				treeCompeleteTop.get(i).add(new DefaultMutableTreeNode(chkLARLT.get(j).get(3)));
				
			}

			treeCompeleteRoot.add(treeCompeleteTop.get(i));
		}
