...
public class CheckList {
	...
	public void addARLChkList(ArrayList<CheckListRecord> recARL) throws Exception{
		...
		//항목저장
		String sql3 = "INSERT INTO test_list (list_num, cat_num, emp_id, content, List_Stat)"
		...
		for(int i = 0; i < recARLTemp.size(); i++){			
			...
			ps2.setString(4, recARLTemp.get(i).getListStat());
			ps2.addBatch();
		}
	...
	/**
	 *  PL용 체크리스트 승인,승인대기,보류 업데이트
	 */
	public void modifyARLChkList(ArrayList<CheckListRecord> recARL) throws Exception{
		
		//1. sql 문장
		String sql2 = "UPDATE test_List SET EMP_ID = ?, LIST_STAT = ? "
				+ " WHERE LIST_NUM = ? AND CONTENT = ? ";
		System.out.println("UPDATE출력: "+ sql2);
		
		//2. sql 전송객체
		ps2 = con.prepareStatement(sql2);
		
		//3. sql 전송. 반복문으로 항목들 addBatch로.
		for(int i = 0; i < recARL.size(); i++){			

			ps2.setString(1, recARL.get(i).getEmpId());
			ps2.setString(2, recARL.get(i).getListStat());
			ps2.setInt(3, recARL.get(i).getListNum());
			ps2.setString(4, recARL.get(i).getContent());
			
			ps2.addBatch();
			
			//참고출력용
			System.out.println(recARL.get(i).getEmpId() + "  " + recARL.get(i).getListStat()
					+ "  " + recARL.get(i).getpNum() + recARL.get(i).getListNum());
	 ...
	 *  PL용 체크리스트 삭제
	 */
	public void delARLChkList(ArrayList<CheckListRecord> recARL) throws Exception{
		
		//1. sql 문장
		String sql2 = "DELETE FROM test_List "
				+ " WHERE LIST_NUM = ? AND CONTENT = ? ";
		System.out.println("DELETE 출력: "+ sql2);
		...
		for(int i = 0; i < recARL.size(); i++){			
			...
			ps2.setInt(1, recARL.get(i).getListNum());
			ps2.setString(2, recARL.get(i).getContent());
	...
	/**
	 *  PL용 체크리스트 현황테이블 갖고 오기
	 */
	public ArrayList<ArrayList<String>> chkLStatTableARL(int p_num, String sw) throws Exception{

		ArrayList<ArrayList<String>> arrPLTableChek = new ArrayList<ArrayList<String>>();
		// 칸 당 arrCtent 한 줄을 담을 ARList
		ArrayList<String> arrPLTableChekCtent;		//카테고리번호, 카테제목, 항목번호, 항목내용, 상태 담을 ARList

		if(sw.equals("삭제용")){
			
			
			System.out.println("갖고올 프로젝트 넘버" + p_num);
			// 6. sql문장 만들기 (select 문장 만들기)
			String sql2 = "SELECT LIST_NUM, CAT_TITLE, CONTENT, EMP_ID, LIST_STAT "
					+   " FROM test_v_cat_List "
					+   " WHERE P_NUM = ? ";
			System.out.println("team select sql문장실행: "+sql2);
			...
			// 8. 전송
			ResultSet getplTableChkList = ps.executeQuery();

			while(getplTableChkList.next()){

				arrPLTableChekCtent = null;
				arrPLTableChekCtent = new ArrayList<String>();

				arrPLTableChekCtent.add(String.valueOf(getplTableChkList.getInt("LIST_NUM")));
				arrPLTableChekCtent.add(getplTableChkList.getString("CAT_TITLE"));
				arrPLTableChekCtent.add(getplTableChkList.getString("CONTENT"));
				arrPLTableChekCtent.add(getplTableChkList.getString("EMP_ID"));
				arrPLTableChekCtent.add(getplTableChkList.getString("LIST_STAT"));

				arrPLTableChek.add(arrPLTableChekCtent);

			}

		}else if(sw.equals("확인용")){
			
			System.out.println("갖고올 프로젝트 넘버" + p_num);
			// 6. sql문장 만들기 (select 문장 만들기)
			String sql2 = "SELECT LIST_NUM, CAT_TITLE, CONTENT, EMP_ID, LIST_STAT "
					+   " FROM test_v_cat_List "
					+   " WHERE P_NUM = ? AND LIST_STAT in('승인대기', '작업중', '보류') ";
			System.out.println("team select sql문장실행: "+sql2);
		...
		ps.close();
		return arrPLTableChek;

	}
	
	/**
	 *  Dev용 개인 체크리스트 현황테이블 갖고 오기
	 */
	public ArrayList<ArrayList<String>> chkLStatTableARL(String Emp_id) throws Exception{

		ArrayList<ArrayList<String>> arrDevTableChek = new ArrayList<ArrayList<String>>();
		// 칸 당 arrCtent 한 줄을 담을 ARList
		ArrayList<String> arrDevTableChekCtent;		//카테고리번호, 카테제목, 항목번호, 항목내용, 상태 담을 ARList

		System.out.println("갖고올 개발자 ID" + Emp_id);
		// 6. sql문장 만들기 (select 문장 만들기)
		String sql2 = "SELECT LIST_NUM, CAT_TITLE, CONTENT, EMP_ID, LIST_STAT "
				+   " FROM test_v_cat_List "
				+   " WHERE Emp_id = ? ";
		...
		while(getplTableChkList.next()){
			...
			arrDevTableChekCtent = null;
			arrDevTableChekCtent = new ArrayList<String>();

			arrDevTableChekCtent.add(String.valueOf(getplTableChkList.getInt("LIST_NUM")));
			arrDevTableChekCtent.add(getplTableChkList.getString("CAT_TITLE"));
			arrDevTableChekCtent.add(getplTableChkList.getString("CONTENT"));
			arrDevTableChekCtent.add(getplTableChkList.getString("EMP_ID"));
			arrDevTableChekCtent.add(getplTableChkList.getString("LIST_STAT"));

			arrDevTableChek.add(arrDevTableChekCtent);

		}

		ps.close();
		return arrDevTableChek;
