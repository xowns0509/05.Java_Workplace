...
//	private String postTitle;	//게시물제목
	...
	public ArrayList<ArrayList<String>> loadBugContent(int p_num) throws Exception{
		...
		//"게시번호", "상태", "제목", "작성자", "작성시간"
		/*String sql = "SELECT POST_NUM, EMP_ID, POST_TIME, POST_STAT, POST_TITLE "
				+ "   FROM test_v_bug_report "
				+ "   WHERE (POST_TYPE = '버그' OR POST_STAT = '공지사항') AND P_NUM = ?";
		*/
		String sql = "SELECT POST_NUM, EMP_ID, POST_TIME, POST_STAT, POST_TITLE "
				+ "   FROM test_v_bug_report "
				+ "   WHERE P_NUM = ?";
	...
	public void saveBoardContent(PostRecord postRec) throws Exception{
		...
		// 3. sql 문장 만들기
		String sql = "Insert into TEST_POST VALUES(seq_tmp_post_POST_NUM, seq_tmp_post_P_NUM, ?, ?, SYSDATE, ?, ?, ?)";
