...
//	JTable devTable, bugTable;
	...
	public DevModel(DevView parentdevView){
		
		this.parentDevView = parentdevView;
	...
	/**
	 * 개발자 개인 체크리스트 정보 갖고 오는 것. 예외만 처리하고 바로 DevView로 던짐.
	 * @param emp_id
	 * @return
	 */
	public ArrayList<ArrayList<String>> devChkLStatTable(){
		
		ArrayList<ArrayList<String>> arrPLTableChek = new ArrayList<ArrayList<String>>();
		// 칸 당 arrCtent 한 줄을 담을 ARList
		//카테고리번호, 카테제목, 항목번호, 항목내용, 상태 담을 ARList
		
		try{

			devchkl = new CheckList();
			arrPLTableChek = devchkl.chkLStatTableARL(parentDevView.getRecord().getEmpId());
	...
	public DefaultMutableTreeNode devChklist(String emp_id){
		...
		for(int i = 0 ; i < arrDevCatTitle.size() ; i++){	//갖고온 카테고리의 갯수만큼만 반복문 돌려.
			...
			devTop.add(new DefaultMutableTreeNode(String.valueOf(i)+". "+arrDevCatTitle.get(i)));
			...
			for (int j = 0; j < arrDevChek.size() ; j++){	//갖고온 체크리스트의 레코드(1세트) 갯수만큼만 돌려.
				...
				if(   (arrDevCatTitle.get(i)).equals(arrDevChek.get(j).get(1)) != false   ){
					...
					devTop.get(i).add(new DefaultMutableTreeNode(arrDevChek.get(j).get(3)));
					//항목내용이 카테고리에 삽입		
				}
			}
