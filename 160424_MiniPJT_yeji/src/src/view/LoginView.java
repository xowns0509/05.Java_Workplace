...
public class LoginView extends JPanel implements ActionListener{
	...
	public LoginView(MileStone parent){
		...
		//아이디, 비밀번호 부분 붙이기 
		JPanel p_id = new JPanel(new FlowLayout(FlowLayout.CENTER));
		p_id.add(lId); p_id.add(tfId);
		JPanel p_pw = new JPanel(new FlowLayout(FlowLayout.CENTER));
		p_pw.add(lPassword); p_pw.add(pfPassword);
		p_north.add(p_id);
		p_north.add(p_pw);
		...
		//로그인, 회원가입 버튼 붙이기 
		JPanel p_south = new JPanel(new GridLayout(1,2,10,0));
	...
	void eventProc(){
		...
		//tfPassword.addActionListener(this); //TODO: 추가 
	...
	public void actionPerformed(ActionEvent e) {
		...
		System.out.println("!");
		//로그인 
		if(evt == bLogin || evt == pfPassword){
			try {
				rec = login.login(tfId.getText(), String.valueOf(pfPassword.getPassword()));
