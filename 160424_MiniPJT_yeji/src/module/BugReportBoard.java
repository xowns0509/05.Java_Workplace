...
//	private String postTitle;	//게시물제목
	...
	//코드게시판 글 불러오는 메소드
	public ArrayList<PostRecord> loadBugContent() throws Exception{
		//p_num 추가 인자로 받기

		ArrayList<PostRecord> postRecARLT = new ArrayList<PostRecord>();
		PostRecord postRec;
		
		// 3. sql 문장만들기
		// 프로젝트 넘버와 게시물종류(코드) 가 필요
		String sql = "SELECT * FROM POST WHERE POST_TYPE = '업무지시' AND P_NUM = ?";
		...
		while(rs.next()){
			...
			postRec = null;
			postRec = new PostRecord();
			
			postRec.setPostNum(rs.getInt("POST_NUM"));
			postRec.setpNum(rs.getInt("P_NUM"));
			postRec.setFileName(rs.getString("P_NUM"));
			postRec.setEmpId(rs.getString("EMP_ID"));
			postRec.setPostTime(rs.getString("POST_TIME"));
			postRec.setPostType(rs.getString("POST_TYPE"));
			postRec.setPostStat(rs.getString("POST_STAT"));
			postRec.setPostTitle(rs.getString("POST_TITLE"));

			postRecARLT.add(postRec);
