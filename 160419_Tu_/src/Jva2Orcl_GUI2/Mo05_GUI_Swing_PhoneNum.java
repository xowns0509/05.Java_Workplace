...
class JEventInfoUI {
	...
	JEventInfoUI(){
		...
		//db연결. 객체명이 아닌 클래스명으로 연결
		try {
			//db = new Mo06_Database(); 이거 대신
			db = Mo06_Database.getInstance();
			ta.setText("db연결성공 !");
		} catch (Exception e) {
	...
	void mdfyData(){
		...
		try{
			int modify = db.modify(new Record2(tftel.getText(), tfname.getText(), tfjumin.getText(), tfage.getText(), tfgender.getText(), tfaddr.getText()));
			ta.setText(modify + "행을 수정했어.");
...
/*	void seeAData(){
	...
	void srchData2(){
		...
		try{
			Record2 rec = db.searchByTel(tel);//Record2의 서치함수 실행.
			
			ta.setText(rec.getName() +" / "+ rec.getTel() +" / "+ rec.getJumin() +" / "+ rec.getGender()+" / "+ rec.getAge()+" / "+ rec.getHome() );
...
//			JButton evt = (JButton)e.getSource();
			if(evt == bInsert){
				...
				JOptionPane.showMessageDialog(null, tftel.getText()+" 를 조회합니다");
				srchData2();
			}
